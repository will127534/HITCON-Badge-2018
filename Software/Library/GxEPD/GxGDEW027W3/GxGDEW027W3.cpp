...
void GxGDEW027W3::_writeToWindow(uint16_t xs, uint16_t ys, uint16_t xd, uint16_t yd, uint16_t w, uint16_t h)
{
  ...
  w = min(w + 7, GxGDEW027W3_WIDTH - xd) + (xd % 8);
  h = min(h, GxGDEW027W3_HEIGHT - yd);
...
{
  w += (x % 8) + 7;
  h = min(h, 256); // strange controller error
...
void GxGDEW027W3::_wakeUp()
{
  ...
  _writeCommand(0x00);
  //writeData(0xaf); //KW-BF   KWR-AF    BWROTP 0f
...
void GxGDEW027W3::drawPagedToWindow(void (*drawCallback)(void), uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  ...
  for (_current_page = 0; _current_page < GxGDEW027W3_PAGES; _current_page++)
  {
    uint16_t yds = max(y, _current_page * GxGDEW027W3_PAGE_HEIGHT);
    uint16_t yde = min(y + h, (_current_page + 1) * GxGDEW027W3_PAGE_HEIGHT);
    if (yde > yds)
    {
      fillScreen(GxEPD_WHITE);
      drawCallback();
