...
void setup(void) {
  ...
  Serial.println("Creat Account Demo!!");
  waitforcmd();
  ...
  {
    Serial.println("Not in Normal Mode! abort.");
    while (1) {
  ...
  if (ret == -1)
  {
    Serial.println("hdw_query_wallet_info Failed!");
    while (1);
  }
  else {
    Serial.println("hdw_query_wallet_info Success");
  }
  ...
  if (ret == -1)
  {
    Serial.println("hdw_create_account_test Failed!");
    while (1);
  }
  else {
    Serial.println("hdw_create_account_test Success");
  }

  ret = hdw_query_account_info();
  if (ret == -1)
  {
    Serial.println("hdw_query_account_info Failed!");
    while (1);
  }
  else {
    Serial.println("hdw_query_account_info Success");
  }

  Serial.println("Generating Address");
  waitforcmd();
  ret = hdw_generate_next_trx_addr(0x00);
  ret = hdw_query_account_key_info(0x00);
  if (ret == -1)
  {
    Serial.println("hdw_query_account_key_info Failed!");
    while (1);
  }
  else {
    Serial.println("hdw_query_account_key_info Success");
  }
}
...
int hdw_create_account_test(void)
{
  int ret = 0;
  uint8_t rcv_buf[64] = { 0 };
