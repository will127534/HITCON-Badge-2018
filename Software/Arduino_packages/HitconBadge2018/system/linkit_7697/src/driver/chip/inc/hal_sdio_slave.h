...
*  - Step 4. Call #hal_sdio_slave_read_mailbox() to read mailbox after SDIO host write mailbox to slave.
*  - Sample code:
*    @code
...
*  - Step 2. Call #hal_pinmux_set_function() to set the GPIO pinmux if the EPT is not in use.
*  - Step 3. Call #hal_sdio_slave_init() to initialize the SDIO slave controller. Call this API only once.
*  - Step 4. Call #hal_sdio_slave_write_mailbox() to write mailbox to SDIO host.
